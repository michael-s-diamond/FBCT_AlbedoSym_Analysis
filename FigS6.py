"""
Code to reproduce Figure S6 in Diamond et al. (2024), GRL
...
for var in ['Ra','Ra_sfc','Ra_aer','Ra_cld']:
    y = ebaf[var].mean(axis=-1)
    ybar = y.mean(axis=0)
    x = np.arange(len(ebaf.time))
    xbar = x.mean()
...
###Plot
#
ylab = [r'$R^\prime$',r'$R^\prime_{\mathrm{sfc}}$',r'$R^\prime_{\mathrm{aer}}$',r'$R^\prime_{\mathrm{cld}}$']
lcolors = ['k',cm.YlOrBr(.33),cm.YlOrBr(.67),cm.Blues(.5)]
lab = ['a','b','c','d']
...
for var in ['Ra','Ra_sfc','Ra_aer','Ra_cld']:
    ...
    plt.plot(np.sin(ebaf.lat.values*np.pi/180),dZt[var],c=lcolors[n-1],lw=3,zorder=11)
    plt.fill_between(np.sin(ebaf.lat.values*np.pi/180),-1*dZe[var],1*dZe[var],facecolor='.75',label=r'$1\sigma$',zorder=-1)
    plt.fill_between(np.sin(ebaf.lat.values*np.pi/180),-2*dZe[var],2*dZe[var],facecolor='.85',label=r'$2\sigma$',zorder=-2)
    plt.fill_between(np.sin(ebaf.lat.values*np.pi/180),-3*dZe[var],3*dZe[var],facecolor='.95',label=r'$3\sigma$',zorder=-3)

    plt.plot(np.sin(ebaf.lat.values*np.pi/180),3*dZe[var],'k',lw=1,ls='dotted')
    plt.plot(np.sin(ebaf.lat.values*np.pi/180),-3*dZe[var],'k',lw=1,ls='dotted')
