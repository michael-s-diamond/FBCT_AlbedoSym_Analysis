...
#Load data
ebaf = xr.open_mfdataset(glob(dir_data+'CERES/EBAFed42/EBAF_decomposition_*.nc')) #processed
...
def geoavg(data,hemi='both',reg='all',W=None):
    ...
    lon, lat = np.meshgrid(ebaf.lon,ebaf.lat)
...
xTr['R_cld'] = (['reg','time'],np.nan*np.ones((nr,nt)))
xTr['R_cld'].attrs = {'long_name' : 'SW CRE time series','units' : 'W m-2'}

xTr['Rc_cld'] = (['reg','month'],np.nan*np.ones((nr,nm)))
xTr['Rc_cld'].attrs = {'long_name' : 'SW CRE climatology (2002-07 to 2022-06)','units' : 'W m-2'}

xTr['Ra_cld'] = (['reg','time'],np.nan*np.ones((nr,nt)))
xTr['Ra_cld'].attrs = {'long_name' : 'Deseasonalized SW CRE anomalies','units' : 'W m-2'}

xTr['Ra_cld_int'] = (['reg'],np.nan*np.ones((nr)))
xTr['Ra_cld_int'].attrs = {'long_name' : 'Rcld anomaly intercept','units' : 'W m-2'}

xTr['Ra_cld_trend'] = (['reg'],np.nan*np.ones((nr)))
xTr['Ra_cld_trend'].attrs = {'long_name' : 'Rcld anomaly trend','units' : 'W m-2 decade-1'}

xTr['Ra_cld_e'] = (['reg','time'],np.nan*np.ones((nr,nt)))
xTr['Ra_cld_e'].attrs = {'long_name' : 'Rcld anomaly residuals','units' : 'W m-2'}

xTr['Ra_cld_nu'] = (['reg'],np.nan*np.ones((nr)))
...
for var in ['R','Ra','S','Sa','R_clr','Ra_clr','R_aer','Ra_aer','R_sfc','Ra_sfc','R_cld','Ra_cld']:
...
for var in ['Rc','Sc','Rc_clr','Rc_aer','Rc_sfc','Rc_cld']:
    for m in range(1,13):
    
        fdata = ebaf[var].sel(month=m).values[np.newaxis,:,:]
...
for var in ['Ra','Sa','Ra_clr','Ra_sfc','Ra_aer','Ra_cld']:
...
###Save file
#
filename = dir_data+'CERES/EBAFed42/EBAF_gavg_trends.nc'
